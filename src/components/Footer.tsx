...
import { useSiteConfig } from '@/hooks/useSiteSettings'
...
export default function Footer() {
  ...
  // URLs par défaut si non configurées
  const facebookUrl = config.facebook_url || 'https://www.facebook.com/profile.php?id=61583076211318'
  const instagramUrl = config.instagram_url || 'https://www.instagram.com/eliati_creations/'
  const tiktokUrl = config.tiktok_url || 'https://www.tiktok.com/@eliati_creations'
  const contactEmail = config.contact_email || 'Contacteliati@gmail.com'
  const siteName = config.site_name || 'EliAti'
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="font-script text-4xl text-leather mb-4">{siteName}</div>
              <p className="text-sm text-taupe leading-relaxed mb-6">
                {config.slogan || 'Bijoux pensés à quatre mains, faits pour durer. Des créations uniques qui accompagnent vos plus beaux moments.'}
              </p>
            ...
            <div>
              ...
              {config.contact_phone && (
                <a 
                  ...
                  className="block mb-4 text-sm text-taupe hover:text-leather transition-colors"
                >
                  📞 {config.contact_phone}
                </a>
              )}
              ...
              {config.address && (
                <p className="text-sm text-taupe mb-6">
                  📍 {config.address}{config.city && `, ${config.postal_code} ${config.city}`}
                </p>
              )}
